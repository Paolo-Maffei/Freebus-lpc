...
void read_value_req(void)	// Empfangenes read_value_request Telegramm verarbeiten
{
	...
	if(objno!=0xFF) {	// falls Gruppenadresse nicht gefunden
		...
		if((objno&0x01)==0)
		{
			objvalue=sendewert(objno); //original objvalue=
...
unsigned int sendewert(unsigned char objno)
{
	...
	{
		eis5temp=temp[objno_help];

		eis5temp=eis5temp*100;

		eis5temp>>=11;



		eis5temp=eis5temp+0x5800;
...
//							send_value(1,(objno<<1),temp[objno]);
...
void restart_app(void)		// Alle Applikations-Parameter zurücksetzen
{
	...
	P0 = 0x20; // 0010 0000*/
	// SIOC
	P0M1= 0x00; // Port 0 output mode 1 0000 0000
	P0M2= 0xFD; // Port 0 output mode 2 1111 1101
	P0 = 0x02; // Port 0 0000 0010
