...
unsigned long read_obj_value(byte objno)
{
	...
	if (objno<12) return  0x66 ; 					//@todo test
	if (objno<14) return  userram[0xFF] ; 			//@todo test
...
void SendRueckmeldung(byte c, byte OldVal)
{
	...
	rs_send_s("\nRueck:");
#endif
		// @-todo Rückmeldungnur sendenwenn Änderung ???
		OldVal;  //zur vermeidung von Warnmeldung solange noch nichtverwendet
		//@todo auchbei Ü-Flag
          ...
          for(c=0;c<2;c++)
            {
              ...
              if(dataw==0&&commObjectNumber==c && sperren[c]==0) //schaltobjekt  aus,
                {
            	  // @todo Softausfunktiion implementieren
...
void CheckKurzschlussSpannungsausfall(byte kanal)
{
	...
	// Check Kurzschluss
	if (kanal==KANAL1) states = P_K_C1;
	else  states = P_K_C2;

	if(states!=IstKurzschluss[kanal])
	{
		send_obj_value(kanal + 14);				// Melden
		if(IstKurzschluss[kanal] = P_K_C1)		// Merken
		{
			RueckmeldungPending[kanal]=true;
			SwitchDimmer(kanal,0,true,0);			// Ausschalten
		}
